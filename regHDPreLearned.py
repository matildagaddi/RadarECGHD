            ...
            for lr in LEARN_RATE: 
                for fs in FS:
                    for sr in SAMPLE_RATE:
                        ...
                        learning_data = train_ds.data #CHANGED
                        initial_input_data = torch.tensor(learning_data).to(device)  #CHANGED
                        model = HDradarECG(f*channels, d, lr, device, initial_input_data).to(device) #CHANGED #f*channels = 400
                        with torch.no_grad():
                        ...
                        plt.legend()
                        plt.savefig(f'prelearn{(mse.compute().item()):.4f}_{corr:.4f}_{d}_{f}_{lr}_{fs}_{PEAK_THRESH}_{datetime.datetime.now()}.png') #need date so it doesn't rewrite previous file
