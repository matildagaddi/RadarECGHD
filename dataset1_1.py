...
class MyDataset(data.Dataset):
    ...
    def __init__(self, path, train, window_size, step_size, device, seed, transform=None, target_transform=None,):
        self.path = path # building the path
        ...
        self.step_size = int(step_size) #number of evenly spaced points sampled from window to not take all points (6000 for 3 seconds)
    ...
    def load_data(self):
        ...
        def process_data(file, radar_data, sequence): 
            file_dict = loadmat(self.path+file) #loads dictionary 
            
            cur_radar_data = {'radar_i': file_dict['radar_i'][sequence[0]+39:sequence[1]:self.step_size], 'radar_q': file_dict['radar_q'][sequence[0]+39:sequence[1]:self.step_size], 
            'tfm_ecg1': torch.FloatTensor(pd.Series(file_dict['tfm_ecg1'].reshape(len(file_dict['tfm_ecg1']))).rolling(40).mean())[sequence[0]+39:sequence[1]:self.step_size]} #causing nan in first 40
            #ecg not smoothed, need to take mean of nearest points (40: phase length before sampling), ecg2 is an amplified version?
            #print(len(cur_radar_data['radar_i']), cur_radar_data['radar_i'])
            ### regHD will handle sliding window ###
            ...
            #standardize center of values #### might remove this to simulate real time 
            stdscaleri = StandardScaler()
            stdscaleri.fit(cur_radar_data['radar_i'])
            cur_radar_data['radar_i'] = torch.FloatTensor(stdscaleri.transform(cur_radar_data['radar_i']))
            #print(len(cur_radar_data['radar_i']), cur_radar_data['radar_i'])
            stdscalerq = StandardScaler()
            stdscalerq.fit(cur_radar_data['radar_q'])
            cur_radar_data['radar_q'] = torch.FloatTensor(stdscaleri.transform(cur_radar_data['radar_q']))

            radar_data['radar_i'] = torch.cat((radar_data['radar_i'], cur_radar_data['radar_i']))
            #print(len(radar_data['radar_i']), radar_data['radar_i'])
            radar_data['radar_q'] = torch.cat((radar_data['radar_q'], cur_radar_data['radar_q']))
            radar_data['tfm_ecg1'] = torch.cat((radar_data['tfm_ecg1'], torch.FloatTensor(cur_radar_data['tfm_ecg1'])))
        ...
        radar_data = {'radar_i': torch.FloatTensor(), 'radar_q': torch.FloatTensor(), 'tfm_ecg1': torch.FloatTensor()}
                                                                        
        if self.train:
            files = ['GDN0006/GDN0006_3_Apnea.mat']#rand_train_files
            ##print('loading training data')
            for file in files:
                process_data(file, radar_data, [3000,13000])
            len_data = (len(radar_data['radar_i']),)
            self.data_i = torch.reshape(radar_data['radar_i'], len_data)#[::self.step_size] #[3000:13000]
            print(len(self.data_i))
            self.data_q = torch.reshape(radar_data['radar_q'], len_data)#[::self.step_size]
            self.target = torch.reshape(radar_data['tfm_ecg1'], len_data)#[::self.step_size]
            
        else:
            files = ['GDN0006/GDN0006_3_Apnea.mat']#rand_test_files
            ##print('loading testing data')
            for file in files:
                process_data(file, radar_data, [13000,23000])
            len_data = (len(radar_data['radar_i']),)
            self.data_i = torch.reshape(radar_data['radar_i'], len_data) #[13000:23000]
            self.data_q = torch.reshape(radar_data['radar_q'], len_data)
            self.target = torch.reshape(radar_data['tfm_ecg1'], len_data)
        ...
        # self.data_i = torch.reshape(data_i, len_data)[::self.step_size]
        # self.data_q = torch.reshape(data_q, len_data)[::self.step_size]
        # self.target = torch.reshape(data_ecg, len_data)[::self.step_size]
