...
class MyDataset(data.Dataset):
    ...
    def load_data(self):
        #exclude files: 1,2,3,15,24,26 no apnea, 20 weird apnea jumps
        # to campare to IABC 80% train 20% test (they said they used 24 to train, but 6 dont have apnea?)
        ...
        #radar_data = {}
        radar_data = {'radar_i': torch.ShortTensor(), 'radar_q': torch.ShortTensor(), 'tfm_intervention': torch.ShortTensor()}

        def process_data(file, radar_data): 
            file_dict = loadmat(path+file) #loads dictionary
            cur_radar_data = {'radar_i': file_dict['radar_i'], 'radar_q': file_dict['radar_q'], 'tfm_intervention': file_dict['tfm_intervention']}
            #print(len(cur_radar_data['radar_i']))
            for k in cur_radar_data.keys():
                ...
                cur_radar_data[k] = torch.ShortTensor(cur_radar_data[k])#change values from array to tensor, #ShortTensor
            ...
            radar_data['tfm_intervention'] = torch.cat((radar_data['tfm_intervention'], cur_radar_data['tfm_intervention']))
            


        if self.train:
            files = rand_train_files
        ...
        labels = radar_data['tfm_intervention']
        # plt.plot(data_i) 
        # plt.plot(labels/3)
        # plt.show()  
        ...
        self.labels = torch.IntTensor(labels3)